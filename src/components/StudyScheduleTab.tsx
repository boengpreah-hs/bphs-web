...
import { Calendar, Clock, Plus, Trash2, Edit, FileText, Globe } from 'lucide-react';
...
interface ScheduleProps {
  ...
  onPostSchedule: (post: AcademicPost) => Promise<void>;
  onDeleteSchedule: (id: string) => Promise<void>;
  onZoomImage: (src: string, allImages?: string[]) => void;
  onViewPdf: (url: string, title: string) => void;
}

export default function StudyScheduleTab({
  dbState,
  isAdminLoggedIn,
  onPostSchedule,
  onDeleteSchedule,
  ...
  const handleDocumentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/upload', {
        ...
        body: JSON.stringify({ file: pFile, name: 'schedule', ext: isPdf ? 'pdf' : 'jpg' })
      });
  ...
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ file: compressed, name: 'schedule_cover', ext: 'jpg' })
      });
      const resData = await response.json();
      if (resData.status === 'success') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const schedulePosts = dbState.schedule_posts || [];
    ...
    const newPost: AcademicPost = {
      ...
      driveLink: driveUrl,
      date: editingId
        ? (schedulePosts.find((x) => x.id === editingId)?.date || new Date().toLocaleDateString('km-KH'))
        : new Date().toLocaleDateString('km-KH') + ' ម៉ោង ' + new Date().toLocaleTimeString('km-KH', { hour: '2-digit', minute: '2-digit', hour12: false }),
    };

    await onPostSchedule(newPost);
    resetForm();
  };

  const schedulePostsList = dbState.schedule_posts || [];
  const poolYears = Array.from(new Set(schedulePostsList.map((x) => x.year))).sort();

  const filteredPosts = schedulePostsList.filter((p) => {
        ...
        <div className="bg-white p-3.5 rounded border border-amber-200 bg-amber-50/15 space-y-3 animate-in fade-in duration-200 text-xs font-battambang text-gray-700">
          <div className="flex justify-between items-center border-b border-amber-200 pb-2">
            <h4 className="font-bold text-amber-800 flex items-center gap-1 font-moul">
              ...
              {editingId ? 'កែសម្រួលកាលវិភាគសិក្សា' : 'បង្ហោះព័ត៌មានកាលវិភាគសិក្សាថ្មី'}
            </h4>
            {editingId && (
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            >
              {uploadBusy ? 'កំពុងដំណើរការ...' : 'រក្សាទុកព័ត៌មាន'}
            </button>
          </form>
        </div>
      )}

      {/* Grid of list matching study schedule styling */}
      {filteredPosts.length === 0 ? (
        <div className="text-center p-12 bg-white rounded border border-gray-150 text-gray-400 text-xs font-battambang">
          មិនទាន់មានសេចក្តីជូនដំណឹងអំពីកាលវិភាគសិក្សាកម្រិតថ្នាក់នេះនៅឡើយទេ
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
          {filteredPosts.map((p) => {
            const defaultCover = `data:image/svg+xml;utf8,${encodeURIComponent(
              `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 297 210' width='297' height='210'><rect width='297' height='210' fill='#f1f5f9'/><text x='148' y='110' font-family="'Kantumruy Pro', sans-serif" font-size='12' font-weight='bold' fill='#1e3a8a' text-anchor='middle'>សេចក្តីជូនដំណឹងកាលវិភាគ</text></svg>`
            )}`;
            ...
            return (
              ...
              >
                <div className="space-y-3">
                  ...
                  {/* Photo or A4 landscape horizontal file preview frame (Aspect A4 Horizontal: 297/210 i.e. 1.414) */}
                  <div
                    className="w-full aspect-[297/210] overflow-hidden rounded border border-gray-200 relative group cursor-pointer"
